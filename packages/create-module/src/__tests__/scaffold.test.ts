...
function opts() {
	return { cwd: tmp, templateDir, resolveVersions: false } as const;
}

describe("scaffoldModule", () => {
	it("creates the module directory", async () => {
		await scaffoldModule("my-test-mod", opts());
	...
	it("throws when the target directory already exists", async () => {
		...
		await expect(scaffoldModule("existing", opts())).rejects.toThrow(
			"Directory already exists",
		);
	});

	it("throws when the name is empty", async () => {
		await expect(scaffoldModule("", opts())).rejects.toThrow(
